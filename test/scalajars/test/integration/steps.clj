(ns scalajars.test.integration.steps
  (:require [kerodon.core :refer :all]
            [clojure.test :as test]
            [clojure.java.io :as io]
            [scalajars.scp :as scp]
...
(defn find-user [key]
  (let [[string user]
        (re-find
         (re-pattern
          (str "command=\"ng --nailgun-port 8700 scalajars.scp (.*)\",no-agent-forwarding,no-port-forwarding,no-pty,no-X11-forwarding " key "\n"))
