...
(deftest added-users-are-added-only-to-their-org-scalajars-group
  (let [email "test@example.com"
        name "testuser"
        password "password"
        ssh-key "asdf"
    ...
    (is (db/add-user email name password ssh-key pgp-key))
    (is (= ["testuser"]
           (db/group-membernames (str "org.scalajars." name))))
    (is (= ["org.scalajars.testuser"]
