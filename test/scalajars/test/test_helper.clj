(ns scalajars.test.test-helper
  (import java.io.File)
  (:require [scalajars.db :as db]
            [scalajars.db.migrate :as migrate]
            [scalajars.config :refer [config]]
            [korma.db :as kdb]
            [clucy.core :as clucy]
            [scalajars.search :as search]
  ...
  (delay
   (let [db (:subname (:db config))]
     (when-not (.exists (io/file db))
       ...
       (sh/sh "sqlite3" db :in (slurp "scalajars.sql"))))
   (migrate/-main)))
